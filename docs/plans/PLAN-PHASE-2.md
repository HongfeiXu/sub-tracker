...
- [x] 点击 FAB "+" 打开底部抽屉，表单字段齐全（名称/金额+币种/周期/日期/分类/颜色/备注）
- [x] 输入"Netflix" → 颜色自动变为红色（#E50914）
    - [BUG] ✅ 已修复：名称输入框右侧添加色块预览
- [x] 选择周期 + 起始日期后，下方显示下次扣款日
- [x] 选择"自定义"周期后出现天数输入框
- [x] 点"+ 新分类"可新建分类，颜色自动分配
- [x] 保存后切到"订阅列表" Tab 可见卡片（品牌色色块 + 首字母 + 名称 + 金额/周期 + 续费日 + 分类）
- [x] 新增一条自定义周期订阅，验证天数输入和折算
- [x] 点击卡片进入编辑模式，数据回填正确
- [x] 编辑订阅修改金额，保存后列表更新
- [x] "标记为已取消" → 卡片移到"已取消"Tab，灰度化显示，显示取消日期
- [x] "重新激活" → 回到"生效中"，nextBillDate 重新计算
- [x] "删除" → 弹出确认弹窗 → 确认后项目消失
    - [BUG] ✅ 已修复：抽屉打开时隐藏 FAB
- [x] 刷新页面，所有数据不丢失
